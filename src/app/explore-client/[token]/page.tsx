...
export default async function ExploreClientRecommendationsPage({ params }: PageProps) {
  ...
  const creator = !isExpired && (list as any)?.createdBy ? ((list as any).createdBy as any) : null;

  const createdAtLabel =
    !isExpired && (list as any).createdAt
  ...
  const expiresAtLabel =
    !isExpired && (list as any).expiresAt
  ...
  const creatorProfileUrl = creator?.publicSlug ? `${siteUrl}/realtor/${creator.publicSlug}` : null;

  const title = !isExpired && (list as any).title ? (list as any).title : "Imóveis selecionados especialmente para você";
        ...
        {isExpired ? (
          <div className="max-w-xl mx-auto mt-10 rounded-2xl border border-gray-200 bg-white p-8 shadow-sm text-center">
            ...
            <p className="text-gray-600 mb-4 text-sm">
              O link de imóveis que você recebeu expirou ou não está mais ativo. Se ainda tiver interesse, você pode pedir um novo link
              diretamente ao profissional que te enviou.
            </p>
            <Link
              href="/"
              ...
              <div className="rounded-3xl bg-gradient-to-r from-sky-600 via-teal-600 to-emerald-500 text-white shadow-lg overflow-hidden">
                <div className="px-6 py-7 md:px-10 md:py-8 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                  <div className="space-y-3 md:space-y-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-white/80 font-semibold">Lista personalizada de imóveis</p>
                    <h1 className="text-2xl md:text-3xl font-bold tracking-tight max-w-2xl">{title}</h1>
                    {message && <p className="text-sm md:text-base text-white/90 max-w-2xl whitespace-pre-line">{message}</p>}
                  ...
                  {creator && (
                    <div className="w-full md:w-auto md:min-w-[260px]">
                      <div className="rounded-2xl bg-white/10 border border-white/20 px-4 py-3 md:px-5 md:py-4 backdrop-blur">
                        ...
                        <p className="text-base md:text-lg font-semibold leading-tight">{creator.name || "Profissional"}</p>
                        {(creator.publicCity || creator.publicState) && (
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2">
                          {creator.publicWhatsApp && (
                            <a
                              href={`https://wa.me/${String(creator.publicWhatsApp).replace(/\D/g, "")}`}
                              target="_blank"
              ...
              {orderedProperties.length === 0 ? (
                <div className="max-w-xl mx-auto mt-6 rounded-2xl border border-dashed border-gray-300 bg-white/60 p-6 text-center">
                  ...
                  <p className="text-xs text-gray-500">
                    Pode ser que eles tenham sido removidos ou estejam temporariamente indisponíveis. Se precisar, fale com o profissional
                    que te enviou o link.
                  </p>
                </div>
              ) : (
