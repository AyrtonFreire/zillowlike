  ...
  try {
    ...
    if (role !== "REALTOR" && role !== "ADMIN") {
      return NextResponse.json(
        { success: false, error: "Você não tem permissão para assumir este lead." },
        { status: 403 },
      );
    }
    ...
    if (lead.status !== "RESERVED" && lead.status !== "WAITING_REALTOR_ACCEPT") {
      return NextResponse.json(
        { success: false, error: "Este lead não está em estado de reserva para ser assumido." },
        { status: 400 },
      );
    }

    const updated = await LeadDistributionService.acceptLead(id, effectiveRealtorId);
    return NextResponse.json({ success: true, lead: updated });
  } catch (error: any) {
    console.error("Error accepting lead:", error);
    return NextResponse.json(
      { success: false, error: error?.message || "Não foi possível assumir este lead agora." },
      { status: 500 },
    );
  }
}
