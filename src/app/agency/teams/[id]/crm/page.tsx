...
export default function AgencyTeamCrmPage() {
  ...
  const handleOpenSuggestedTemplates = (lead: PipelineLead, member: PipelineMember | null, templateCtx: any) => {
    ...
    const defaultChannel: "CHAT" | "WHATSAPP" = assignedId ? "CHAT" : wa ? "WHATSAPP" : "CHAT";
    ...
    setTemplateModal({
      ...
      options: [
        {
          id: "recommended",
          label: "Sugestão recomendada",
          text: buildAgencyLeadRecommendedMessage(templateCtx),
        },
        ...options,
      ],
    });
  ...
  const handlePrioritizeLead = async (lead: PipelineLead, pendingAt?: string | null) => {
    ...
    if (!assignedId) {
      setError("Atribua um corretor antes de registrar um acompanhamento para este lead.");
      return;
    }

    const title = pendingAt ? "Cobrar atendimento do lead" : "Reforçar próximo passo";
    const message = pendingAt
      ? `${lead.contact?.name || "Este lead"} segue aguardando retorno do corretor ${lead.realtor?.name || "responsável"}.`
      : `${lead.contact?.name || "Este lead"} precisa de acompanhamento comercial para acelerar o próximo passo.`;
    ...
    try {
      ...
      const res = await fetch("/api/agency/notices", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          teamId,
          realtorId: assignedId,
          leadIds: [leadId],
          title,
          message,
        }),
      ...
      const data = await res.json().catch(() => null);
      if (!res.ok || !data?.success) {
        throw new Error(data?.error || "Não conseguimos registrar este acompanhamento agora.");
      }

      setSuccessMessage("Acompanhamento enviado ao corretor responsável.");
      await reload({ append: false });
    } catch (e: any) {
      setError(e?.message || "Não conseguimos registrar este acompanhamento agora.");
    } finally {
                    ...
                    <tr key={leadId} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => handleOpenSuggestedTemplates(l, member, templateCtx)}
                            ...
                            title="Abrir mensagens sugeridas"
                            aria-label="Abrir mensagens sugeridas"
                          >
                          ...
                          <button
                            ...
                            disabled={prioritizingLeadId === leadId || !assignedId}
                            ...
                            title={assignedId ? "Registrar acompanhamento com o corretor" : "Atribua um corretor para registrar acompanhamento"}
                            aria-label="Registrar acompanhamento operacional"
                          >
            ...
            <div className="absolute inset-0 flex items-center justify-center p-4">
              ...
              >
                <div className="px-5 py-4 border-b border-gray-200">
                  <div className="text-sm font-semibold text-gray-900">Mensagens sugeridas</div>
