...
const CRECI_RULES: Record<string, { minDigits: number; maxDigits: number; prefix?: string }> = {
...
export function validateCRECIFormat(creci: string): boolean {
  // Formato aceito: 4-6 dígitos, opcionalmente seguido de -F (pessoa física) ou -J (pessoa jurídica)
  const creciRegex = /^\d{4,6}(-[FJ])?$/;
...
export function validateCRECI(creci: string, state: string): CRECIValidation {
  ...
  const match = creci.match(/^(\d{4,6})(-[FJ])?$/);
  if (!match) {
  ...
  // Avisos (não impedem aprovação, mas alertam o admin)
  if (!suffix) {
