...
export default function RecoveryEmailModal({
  ...
  const sendCode = async (targetEmail?: string) => {
    ...
    try {
      const res = await fetch("/api/recovery-email/send-code", {
        method: "POST",
  ...
  const verifyCode = async (fullCode: string) => {
    ...
    try {
      ...
      const newEmail = String(data?.recoveryEmail || "").toLowerCase().trim();
          ...
          >
            <div className="relative bg-gradient-to-r from-teal-600 to-teal-500 px-6 py-7 text-white">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-semibold">E-mail de recuperação</h2>
                  <p className="text-white/80 text-sm">Proteja sua conta com um e-mail alternativo</p>
                </div>
              </div>
            </div>
            ...
            <div className="px-6 py-6">
              ...
              {step === "edit" && (
                <div className="space-y-4">
                  <div className="text-sm text-gray-600">
                    E-mail de recuperação atual:
                    <div className="mt-1 font-semibold text-gray-900 break-all">
                      {currentRecoveryEmail ? String(currentRecoveryEmail).toLowerCase().trim() : "(não configurado)"}
                    </div>
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Novo e-mail de recuperação</label>
                    <input
                      type="email"
                      value={email}
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      Vamos enviar um código para confirmar. Ele expira em 10 minutos.
                    </p>
                  </div>
              ...
              {step === "success" && (
                <div className="text-center py-10">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">E-mail de recuperação confirmado!</h3>
                  <p className="text-gray-600 text-sm">Guarde esse e-mail com segurança.</p>
                </div>
              )}
            </div>
          </motion.div>
