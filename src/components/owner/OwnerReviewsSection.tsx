...
export default function OwnerReviewsSection({ ownerId, initialAvgRating = 0, initialTotalRatings = 0 }: Props) {
  ...
  const canSeeAll = Boolean(sessionUserId) && (sessionRole === "ADMIN" || sessionUserId === ownerId);
  ...
  const buildQuery = useCallback(
    ...
    [filterHasComment, filterStars, ownerId, canSeeAll]
  );

  const fetchEligibility = useCallback(async () => {
    try {
      const res = await fetch(`/api/owner-ratings/eligibility?ownerId=${encodeURIComponent(ownerId)}`, { cache: "no-store" });
  ...
  const fetchRatings = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/owner-ratings?${buildQuery(null)}`, { cache: "no-store" });
  ...
  const fetchMore = useCallback(async () => {
    ...
    setLoadingMore(true);
    try {
      const res = await fetch(`/api/owner-ratings?${buildQuery(nextCursor)}`, { cache: "no-store" });
      const data = (await res.json().catch(() => null)) as RatingsResponse | null;
      if (!res.ok || !data) {
        return;
      }
  ...
  const submitReply = async (ratingId: string) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/owner-ratings/reply", {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  const reportReview = async (ratingId: string) => {
    ...
    try {
      const res = await fetch("/api/review-reports", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ targetType: "OWNER", ratingId, reason: "OTHER" }),
      });
      const data = await res.json().catch(() => null);
      if (!res.ok || !data?.success) {
        ...
        <div className="lg:col-span-2">
          {loading ? (
            ...
            <div className="space-y-4">
                  ...
                  <div key={r.id} className="rounded-xl border border-gray-200 p-4">
                    ...
                    {r.replyText && (
                      <div className="mt-3 rounded-lg bg-gray-50 border border-gray-200 p-3">
                        <div className="text-xs font-semibold text-gray-700">Resposta do anunciante</div>
                    ...
                    {replyOpenForId === r.id && (
                      <div className="mt-3">
                        <textarea
                          className="w-full rounded-lg border border-gray-300 p-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
                          rows={3}
                          value={draftReply}
                        ...
                        <div className="mt-2 flex items-center justify-end gap-2">
                          ...
                          <Button variant="primary" size="sm" disabled={submitting} onClick={() => submitReply(r.id)}>
                            Enviar
                          </Button>
                        </div>
                      </div>
              ...
              {nextCursor && (
                <div className="pt-2">
                  <Button variant="secondary" size="sm" disabled={loadingMore} onClick={fetchMore}>
      ...
      {writeOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6">
            ...
            <p className="text-sm text-gray-600 mt-1">Sua avaliação é pública e deve refletir sua experiência real.</p>

            <div className="mt-4">
              <div className="text-sm font-semibold text-gray-900 mb-2">Nota</div>
              <RatingStars rating={draftStars} onRate={setDraftStars} size="md" />
            </div>

            <div className="mt-4">
              <div className="text-sm font-semibold text-gray-900 mb-2">Comentário (opcional)</div>
              <textarea
                className="w-full rounded-lg border border-gray-300 p-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
                rows={4}
                value={draftComment}
            ...
            <div className="mt-5 flex items-center justify-end gap-2">
              <Button variant="secondary" disabled={submitting} onClick={() => setWriteOpen(false)}>
                Cancelar
              </Button>
              <Button variant="primary" disabled={submitting} onClick={submitReview}>
                Enviar
              </Button>
            </div>
          </div>
