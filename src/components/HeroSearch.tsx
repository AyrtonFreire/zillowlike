...
function LastSearchRow() {
  ...
  if (!label) return (
    <div className="card p-4 text-sm text-gray-700 flex items-center gap-2">
  ...
  return (
    <div className="card p-4 text-sm text-gray-700 flex items-center gap-2">
      ...
      <button
        className="ml-auto btn btn-ghost text-xs px-3 py-1"
      ...
      <div className="relative z-10 h-full">
        <div className="mx-auto max-w-7xl px-4 lg:px-6 h-full flex items-center">
          <div className="w-full max-w-3xl">
            ...
            {/* Dropdown de exploração */}
            {openPanel && (
              <div ref={panelRef} className={`mt-6 card overflow-hidden transform transition-all duration-200 ease-out origin-top ${panelIn ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2'}`}>
                {!showAdvanced ? (
                  <div className="flex flex-col">
                    ...
                    <div className="px-6 pt-4">
                      <div className="flex items-center gap-7 text-sm">
                        {(['buy','rent','share','sold'] as const).map(t => (
                          <button key={t} onClick={()=>setPanelTab(t)} className={`pb-2 font-medium transition-colors ${panelTab===t ? 'text-primary-700 border-b-2 border-primary-600' : 'text-gray-700 hover:text-gray-900 border-b-2 border-transparent'}`}>
                            {t==='buy'?'Comprar':t==='rent'?'Alugar':t==='share'?'Compartilhar':'Vendidos'}
                            {t==='sold' && <span className="ml-2 badge badge-success text-xs">NOVO</span>}
                          </button>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="px-6 mt-2 pb-5">
                                ...
                                {['Petrolina','Juazeiro'].map((c)=>(
                                  <li key={c} className="py-2">
                                    <div className="flex items-center justify-between">
                                      <button type="button" className="text-gray-800 hover:text-primary-700 font-medium" onClick={()=> goToCity(c)}>{c}</button>
                                      <button type="button" className="text-primary-600 hover:text-primary-700 text-sm" onClick={()=> setExploreView(c==='Petrolina'?'areas:petrolina':'areas:juazeiro')}>Áreas em {c}</button>
                                    </div>
                                  </li>
                                ))}
                              </ul>
                        ...
                        {exploreView !== 'root' && (
                          <div className="text-sm">
                            <div className="mx-auto w-full max-w-2xl px-2 py-6">
                              ...
                              <div className="text-gray-900 font-semibold mb-4">{exploreView === 'areas:petrolina' ? 'Bairros de Petrolina' : 'Bairros de Juazeiro'}</div>
                              <ul className="space-y-2">
                                {(exploreView === 'areas:petrolina' ? AREAS.Petrolina : AREAS.Juazeiro).map((a)=> (
                                  <li key={a} className="py-1">
                                    <button type="button" className="text-gray-800 hover:text-primary-700" onClick={()=>{ setQ(a); }}>
                                      {a}
                                    </button>
                                  </li>
                                ))}
                      ...
                      <div className="mx-auto w-full max-w-2xl flex items-center justify-end gap-3 mt-4 px-2">
                        <button type="button" onClick={()=> setOpenPanel(false)} className="btn btn-secondary">Fechar</button>
                  ...
                  <div className="p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <label className="block">
                        <div className="text-sm font-medium text-gray-800 mb-2">Cidade</div>
                      ...
                      <label className="block">
                        <div className="text-sm font-medium text-gray-800 mb-2">Valor máximo</div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <label className="block">
                        <div className="text-sm font-medium text-gray-800 mb-2">Tipo</div>
                      ...
                      <label className="block">
                        <div className="text-sm font-medium text-gray-800 mb-2">Quartos</div>
                      ...
                      <label className="block">
                        <div className="text-sm font-medium text-gray-800 mb-2">Banheiros</div>
                    ...
                    <div className="flex items-center justify-between pt-2">
                      <button type="button" onClick={()=> setShowAdvanced(false)} className="btn btn-ghost">Voltar</button>
